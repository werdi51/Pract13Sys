...
DWORD WINAPI Player(LPVOID param)
{
    ...
    while (_playerStatus->IsAlive && Game)
    {
        ...
        DWORD ULT = WaitForSingleObject(BOSS_SPECIAL_EVENT, 100);
        ...
        strcpy_s(_playerStatus->Status, "Перезаряжаюсь");
...
int main()
{
    ...
    PLAYER_ATTACK = CreateEvent(NULL, TRUE, FALSE, NULL);
    PLAYER_SPECIAL = CreateEvent(NULL, TRUE, FALSE, NULL);
    BOSS_ATTACK = CreateEvent(NULL, TRUE, FALSE, NULL);
