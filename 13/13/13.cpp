...
void BossToPlayer(PlayerStatus* player, BossStatus* boss) {
    ...
    if (roll > player->PStats.dodgeChance) {
        ...
        strcpy_s(player->Status, "Атакован босср ");
    }
    else {
...
void BossToPlayers(PlayerStatus* player, BossStatus* boss) {
    ...
    strcpy_s(player->Status, "Ранен спецом");
...
int main() {
    ...
    _bossStatus.BStats.health = 500000 + rand() % 9000000;
    ...
    if (!Drawler) {
        cout << "Ошибка отрисовки " << GetLastError();
        return GetLastError();
    }
